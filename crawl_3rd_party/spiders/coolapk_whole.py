...
class CoolapkWholeSpider(CrawlSpider):
    ...
    def parse_item(self, response, category):
        ...
        version = response.xpath('//p[@class="detail_app_title"]/span/text()').extract_first()
        if version:
            item["Version"] = version.strip()
        else:
            item["Version"] = ''
        item["ID"] = "Coolapk_" + response.request.url.split('/')[-1] + "_v" + item["Version"]
        ...
        item["file_type"] = '.apk'
        installs = response.xpath('/html/body/div/div[2]/div[2]/div[1]/div/div/div[1]/p[2]/text()').extract_first().split('/')[1]
